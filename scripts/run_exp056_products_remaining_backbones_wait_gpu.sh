...
EXP_ROOT="experiments/exp056_ogbn_products_remaining_backbones_sample300e"
...
find_free_gpu() {
  gpu_used_table | while IFS=',' read -r raw_idx raw_used; do
    ...
    if [[ -n "$idx" && -n "$used" && "$used" -le "$FREE_MEM_MIB" ]]; then
      echo "$idx"
      return 0
    fi
  done
...
echo "[$(date '+%F %T')] EXP056 launcher started."
...
for item in "${BACKBONE_ROUTES[@]}"; do
  ...
  summary_csv="$out_dir/ogbn-products_summary.csv"
...
echo "[$(date '+%F %T')] EXP056 launcher finished."
